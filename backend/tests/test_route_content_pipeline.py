...
def test_agent_dry_run_writes_prompt_and_metadata_without_output(
    ...
    run_metadata = json.loads(
        (route_dir / "brief_to_dossier-run.ai-draft.json").read_text(encoding="utf-8")
    )
    assert exit_code == 0
...
def test_agent_renew_writes_prompt_backup(tmp_path: Path) -> None:
    ...
    assert (prompt_path.with_name("brief_to_dossier-prompt.ai-draft.md.bak")).read_text(
        encoding="utf-8"
    ) == "manual prompt\n"
...
def test_agent_prompts_include_editorial_quality_contracts(tmp_path: Path) -> None:
    ...
    framing_prompt = (route_dir / "concept_to_event_framing-prompt.ai-draft.md").read_text(
        encoding="utf-8"
    )
    revision_prompt = (route_dir / "validation_to_revision_plan-prompt.ai-draft.md").read_text(
        encoding="utf-8"
    )
...
def test_accepted_events_step_generates_draft_from_keep_candidates(tmp_path: Path) -> None:
    ...
    write_event_list_decisions(
        content_root, seed_dir, route_dir, {"kool-herc-sedgwick-party": "keep"}
    )
...
def test_unresolved_merge_candidate_blocks_accepted_events_generation(tmp_path: Path) -> None:
    ...
    route_dir = content_root / ROUTE_ID
    write_event_list_decisions(
        content_root, seed_dir, route_dir, {"kool-herc-sedgwick-party": "merge"}
    )
        ...
        "accepted_events": [
            {
                ...
                "quality_check": {
                    field: True for field in route_content_pipeline.QUALITY_GATE_FIELDS
                },
            },
            {
...
def write_pipeline_fixture(tmp_path: Path) -> tuple[Path, Path]:
    ...
    content_docs.mkdir()
    (content_docs / "route-research-dossier-template.md").write_text(
        "# Template\n", encoding="utf-8"
    )
    (content_docs / "route-editorial-quality-standards.md").write_text(
        "# Standards\n", encoding="utf-8"
    )
    route_dir = content_root / ROUTE_ID
